...
export async function createApp() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self'; style-src 'self'; media-src 'self'; connect-src 'self'; base-uri 'none'; frame-ancestors 'none'");
    try {
      if (req.headers.origin && req.headers.origin !== `http://${req.headers.host}` && req.headers.origin !== `https://${req.headers.host}`) return json(res, 403, { error: 'Cross-origin requests are not allowed' });
      ...
      const match = url.pathname.match(/^\/api\/rooms\/([a-fA-F0-9]{6})(?:\/(events|tracks|control)(?:\/([a-f0-9-]+))?)?$/);
      if (match) {
        ...
        if (req.method === 'POST' && action === 'control') {
          ...
          if (!room.current) return json(res, 400, { error: 'Upload a track first.' });
          room.position = position(room);
          room.updatedAt = Date.now();
          if (data.action === 'play') room.playing = true;
          else if (data.action === 'pause') room.playing = false;
          else if (data.action === 'seek' && Number.isFinite(data.position) && data.position >= 0 && data.position <= 86400) room.position = data.position;
    ...
    } catch (error) { if (!res.headersSent && !res.destroyed) json(res, 400, { error: 'Could not complete the request.' }); }
  });
  const timer = setInterval(() => {
    for (const [code, room] of rooms) {
      ...
      if (!room.clients.size && !room.uploads && Date.now() - room.touched > 24 * 60 * 60 * 1000) {
        rooms.delete(code);
        for (const track of room.tracks) { storage -= track.size; rm(path.join(directory, track.id), { force: true }).catch(() => {}); }
      }
    }
  }, 15000);
  timer.unref();
  server.on('close', () => { clearInterval(timer); rm(directory, { recursive: true, force: true }).catch(() => {}); });
  return server;
}
