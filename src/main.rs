...
.context-menu button, .context-menu a { display: block; width: 100%; padding: 9px 10px; border: 0; border-radius: 7px; background: #ffffff !important; color: #111827 !important; font: inherit; font-weight: 650; text-align: left; text-decoration: none; cursor: pointer; appearance: none; }
.context-menu button:hover, .context-menu a:hover, .context-menu button:focus, .context-menu a:focus { background: #ffffff !important; color: #111827 !important; outline: 1px solid #d9dee7; text-decoration: none; }
.context-menu button:active, .context-menu a:active { background: #ffffff !important; color: #111827 !important; }
.context-menu .danger, .context-menu .danger:hover, .context-menu .danger:focus, .context-menu .danger:active { color: #991b1b !important; }
...
fn serve_directory(
    ...
    if !relative.as_os_str().is_empty() {
        ...
        body.push_str(r#""><span class="name-cell"><a class="name" href=""#);
        body.push_str(&escape_html(&parent_url_path));
        body.push_str(r#""><span class="icon">DIR</span><span>..</span></a></span><span class="type">Parent folder</span><span>-</span><span>-</span></div>"#);
    }
    ...
    for entry in entries {
        ...
        if display_name.ends_with(REMOTE_LINK_SUFFIX) {
            if let Ok(contents) = fs::read_to_string(entry.path()) {
                if let Some(link) = RemoteLink::from_text(&contents) {
                    ...
                    body.push_str(r#"><span class="name-cell">"#);
                    if can_write_here {
                        body.push_str(
                            r#"<input class="select-box" type="checkbox" aria-label="Select "#,
                        );
                        body.push_str(&escape_html(&link.name));
                        body.push_str(r#"">"#);
                    }
                    ...
                    body.push_str(&escape_html(&link.name));
                    body.push_str(r#" ↗</span></a></span><span class="type">Remote link</span><span>-</span><span>-</span></div>"#);
                    continue;
                }
            }
        }
        ...
        body.push_str("\"><span class=\"name-cell\">");
        if can_drag {
            body.push_str("<input class=\"select-box\" type=\"checkbox\" aria-label=\"Select ");
            body.push_str(&escape_html(&display_name));
            body.push_str("\">");
        }
        ...
        body.push_str("</span></a></span><span class=\"type ");
